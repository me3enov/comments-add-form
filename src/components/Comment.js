export class Comment {
  ...
  constructor(item, currentUserId, formatDate, { handleBinClick, handleLikeClick }) {
    ...
    this._date = item.date === undefined || item.date === '' ? new Date() : new Date(item.date);
  ...
  _getTemplate() {
    const commentElement = document
      ...
      .content.querySelector(this._commentSelector)
      .cloneNode(true);
  ...
  generateComment() {
    ...
    if (this._link !== undefined && this._link !== '')
      this._avatarElement.style.backgroundImage = `url(${this._link})`;
